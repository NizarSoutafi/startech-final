...
export default function Dashboard() {
  ...
  const projectId = params?.projectId as string || "GUEST"
    ...
    const interval = setInterval(() => {
      if (videoRef.current && canvasRef.current && newSocket.connected) {
        ...
        if (ctx) {
          ...
          const mediaTime = mediaVideoRef.current ? mediaVideoRef.current.currentTime : 0
          newSocket.emit('process_frame', { image: dataUrl, mediaTime })
        }
      }
    }, 200)
  ...
  const handleLogin = (e: React.FormEvent) => { 
    ...
    if (formData.firstName && formData.lastName) {
      setUserInfo({ firstName: formData.firstName, lastName: formData.lastName, clientId: projectId })
    } 
  }
  ...
  const renderMediaContent = () => {
    ...
    switch (mediaType) {
      case 'video':
        return <video ref={mediaVideoRef} src={mediaUrl} controls className="w-full max-h-full rounded shadow-sm" />
      case 'image':
      ...
      <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4 relative overflow-hidden text-slate-900">
        ...
        <Card className="w-full max-w-md border-slate-200 bg-white shadow-2xl relative z-10">
          <CardHeader className="text-center space-y-4">
            ...
            <div>
              <CardTitle className="text-3xl font-bold tracking-tight text-slate-900">STARTECH <span className="text-green-600">ID</span></CardTitle>
            </div>
          </CardHeader>
          <form onSubmit={handleLogin}>
            ...
            <div className="p-6 pt-0">
              <Button type="submit" className="w-full bg-green-600 hover:bg-green-700 text-white h-12 text-lg font-bold">COMMENCER LE TEST</Button>
            </div>
          </form>
        </Card>
      </div>
      ...
      <header className="border-b border-slate-200 bg-white/80 backdrop-blur-md sticky top-0 z-50 shadow-sm">
        <div className="flex h-16 items-center px-4 md:px-6 justify-between">
          <div className="flex items-center gap-3 font-bold text-xl tracking-tight text-slate-900">
            ...
            STARTECH <span className="text-slate-400 font-normal hidden md:inline">VISION</span>
          </div>
        ...
        <div className="w-full flex flex-col gap-4">
          ...
          <div className="p-4 rounded-xl bg-white border border-slate-200 shadow-sm flex justify-between items-center text-xs font-mono text-slate-500">
            <div className="flex items-center gap-2">
              <Zap className={`w-3 h-3 ${isConnected ? "text-green-500" : "text-red-500"}`} />
              SERVEUR STARTECH
            </div>
            <span className={isConnected ? "text-green-600 font-bold bg-green-50 px-2 py-1 rounded" : "text-red-500 bg-red-50 px-2 py-1 rounded"}>
